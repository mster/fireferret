      ...
      const tickStream = async function (ID, remainder) {
        process.nextTick(async function () {
          const next = async function () {
            ...
            try {
              const cursor = await _collection.find(ID ? { ...query, _id: { $gt: ID } } : query, options)
              return cursor.limit(_options.streamDocumentSkip > remainder ? remainder : _options.streamDocumentSkip).toArray()
            } catch (err) {
              return null
            }
          }
          ...
          if (nextBatch) {
            const lastId = nextBatch[nextBatch.length - 1] ? nextBatch[nextBatch.length - 1]._id : null
            ...
            const buffer = Buffer.from(`${firstWrite ? '' : ','}` + JSON.stringify(nextBatch).slice(1, -1), _options.encoding)
            docStream.write(buffer)
            firstWrite = false

            tickStream(lastId, _options.streamDocumentSkip > remainder ? 0 : remainder - _options.streamDocumentSkip)
            return
          }
