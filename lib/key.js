...
const {
  SPECIAL_CHARS: { BUCKET_DELIMITER, QUERY_DELIMITER },
...
class QueryKey {
  ...
  queryString () {
    if (!this._queryString) {
      this._queryString = JSON.stringify(this.query)
    }
...
class BucketKey {
  ...
  baseKey () {
    if (!this._baseKey) {
      this._baseKey = `${this.ns}:${this.dbName}::${this.collectionName}:bucket:`
    }
  ...
  toString () {
    if (!this._toString) {
      this._toString = `${this.ns}:${this.dbName}::${this.collectionName}:bucket${BUCKET_DELIMITER}${this.bucketSize};${this.firstID};${this.lastID}`
    }
...
module.exports = { QueryKey, BucketKey }
