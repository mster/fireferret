...
const { validatePaginationOpts } = require('./utils')
  ...
  toString (baseKeyOnly = false) {
    ...
    try {
      ...
      if (
        ...
        const stringKey = `${this.ns}:${this.dbName}::${this.collectionName}:${stringQuery}::${stringFilteredOpts}`
      ...
      /* Base key */
      const stringKey = `${this.ns}:${this.dbName}::${this.collectionName}:${stringQuery}`
      return stringKey
    } catch (err) {
      throw new FFError(
        'InternalError',
