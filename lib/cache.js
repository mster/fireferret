...
class Cache {
   ...
   * @param {RedisClient} client - An npm::redis client that has already been connected.
   */
  _setClient (client) {
   ...
   */
  async getDocuments (queryList, queryKey, options) {
    if (!options) options = {}

    const cacheOps = this.debucketify(queryList, queryKey.collectionName)
    ...
    if (!options.stream) {
      ...
      const map = options.hydrate ? hydrate : JSON.parse
      return matflap(rawDocuments, map)
    }
  ...
  async setDocuments (queryKey, documents) {
    ...
    for (let i = 0; i < hashes.length; i++) {
      this.hmset(
        `${queryKey.collectionName}:${hashes[i]}`,
        buckets[hashes[i]]._
      )
    }
   ...
   */
  async getDocument (documentID, collectionName) {
    const hash = `${collectionName}:${hashFunction(documentID)}`
   ...
   * @returns {void}
   */
  async setDocument (document, requestedID, collectionName) {
    const documentID =
    ...
    debug(`Setting document into bucket: ${hash}`)
    this.hset(hash, args)
  }
  ...
  async getWideMatch (queryKey) {
    ...
    if (cachedQueries && cachedQueries.length > 0) {
      ...
      if (targetQuery && rangeOptions) {
        debug('Valid Wide-Match was found!')
        return this.lrange(
          targetQuery,
          rangeOptions.start,
          rangeOptions.end - 1
    ...
    debug('No valid Wide-Matches were found.')
    return null
  }
   ...
   */
  debucketify (queryList, collectionName) {
