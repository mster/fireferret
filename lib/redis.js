...
class RedisClient {
  ...
  async multihmget (operations, stream) {
    ...
    const tickWrite = async (
      ...
      process.nextTick(
        async function (writable, operation, args) {
          const payload = await operation(...args)

          let i = 0
          const length = payload.length
          const lastDocument = payload[length - 1]
          for (; i < length - 1; i++) writable.write(payload[i] + '\n')

          if (isLast) writable.end(lastDocument)
          else writable.write(lastDocument + '\n')
        },
        writable,
        operation,
        args
