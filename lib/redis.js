...
class RedisClient {
  ...
  async multihmget (operations, stream) {
    ...
    /* using streams */
    async function writeOnTick (writable, fn, args, last) {
      const payload = await fn(...args)

      for (let i = 0; i < payload.length - 1; i++) {
        writable.write(payload[i] + '\n')
      }

      if (last) writable.end(payload[payload.length - 1])
      else writable.write(payload[payload.length - 1] + '\n')
    }

    const tickWrite = async function (writable, fn, args, last, sleepForTicks) {
      if (sleepForTicks > 0) {
        process.nextTick(
          tickWrite,
          writable,
      ...
      writeOnTick(writable, fn, args, last)
    }
    ...
    for (let i = 0; i < hashes.length; i++) {
      const hash = hashes[i]
      const keys = operations[hashes[i]]
      tickWrite(source, this.hmget, [hash, keys], i === hashes.length - 1, i)
    }
  ...
  async batchlpush (key, elements, batchSize = _options.batchSize) {
    ...
    for (let i = 0; i < Math.ceil(elements.length / batchSize); i += 1) {
      const batch = sliced(elements, i * batchSize, (i + 1) * batchSize)
