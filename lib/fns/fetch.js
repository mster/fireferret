...
module.exports.fetch = async function (query, options) {
  ...
  switch (initalVerdict) {
    case CACHE_HIT:
      return cacheHit.bind(this)(idMap, options)
    case EMPTY_QUERY:
      return []
    case CACHE_MISS:
      ...
      if (this.options.wideMatch) {
        ...
        switch (wideMatchVerdict) {
          case CACHE_HIT:
            this.redis.lpush(sourceKey, wideMatch.reverse())
            return cacheHit.bind(this)(wideMatch, options)
          case EMPTY_QUERY:
            return []
        }
      }
...
async function cacheHit (ids, options = {}) {
  ...
  const batch = await this.redis.batchhgetall(ids)
  const docs = []

  /* (docsCount / batchSize) operations */
  for (let i = 0; i < batch.length; i++) { docs.push(...batch[i].filter(e => e != null)) }
...
async function cacheMiss (query, sourceKey, options = {}) {
  ...
  if (options.stream) {
    const docStream = this.mongo.findDocsStream(query, mongoQueryOptions)

    let chunks = ''
    docStream.on('data', chunk => { chunks += chunk })
    docStream.on('end', () => {
      const parsed = JSON.parse(chunks.trim())
      ...
      if (ids.length === 0) {
        ...
        this.redis.batchlpush(sourceKey, ids)
        this.redis.batchhset(hashes)
      }
    })
